...
const postinstall = async (baseDir) => {
    ...
    let isLerna = false;
    // lerna support
    if (fs_1.existsSync(path_1.join(baseDir, 'lerna.json'))) {
...
const getLernaPackagesJson = () => {
    const pkgJsonList = [];
    try {
        const originData = child_process_1.execSync('npx lerna ls --json').toString();
...
const getPkgJson = (dirPath) => {
    const pkgFile = path_1.join(dirPath, 'package.json');
    if (!fs_1.existsSync(pkgFile)) {
        return;
    }
    return JSON.parse(fs_1.readFileSync(pkgFile).toString());
};
