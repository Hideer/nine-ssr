...
function writeWrapper(options) {
    var _a, _b, _c;
    const { service, distDir, starter, baseDir, cover, faasModName = '@midwayjs/faas', initializeName, advancePreventMultiInit, loadDirectory = [], preloadModules = [], faasStarterName, middleware, clearCache = true, templatePath, moreArgs, isDefaultFunc = false, skipInitializer = false, preloadFile, entryAppDir, entryBaseDir, } = options;
    const files = {};
    ...
    const tpl = (0, fs_1.readFileSync)(templatePath
        ? templatePath
        : (0, path_1.resolve)(__dirname, isCustomAppType
            ? '../wrapper_app.ejs'
            : isMoreThanFaaS2
                ? '../wrapper_bootstrap.ejs'
                : '../wrapper.ejs')).toString();
