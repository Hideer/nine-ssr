...
const formatUpperCamel = str => {
    return exports.firstCharLower(str).replace(/[A-Z]/g, match => `-${match.toLowerCase()}`);
};
...
const docopy = async (sourceDir, targetDir, paths, log) => {
    ...
    await Promise.all(paths.map((path) => {
        const source = path_1.join(sourceDir, path);
        const target = path_1.join(targetDir, path);
        if (fs_extra_1.existsSync(target)) {
            const sourceStat = fs_extra_1.statSync(source);
            const targetStat = fs_extra_1.statSync(target);
