...
const analysis = async (codePath) => {
    ...
    return exports.analysisResultToSpec(analysisResult);
};
...
const formatFuncInfo = (result, funcInfo, parentTarget) => {
    ...
    if (typeof params[0] === 'string') {
        ...
        handler = `${utils_1.formatUpperCamel(className)}.${utils_1.formatUpperCamel(funcName)}`;
        trigger = params[0];
    }
    ...
    if (trigger.event) {
        ...
        if (eventType === 'http') {
            event.http = {
                ...
                path: trigger.path ||
                    `/${utils_1.firstCharLower(className)}/${utils_1.firstCharLower(funcName)}`,
            };
        }
        // 防止有重复的触发器
        const currentEventKey = utils_1.getEventKey(eventType, event[eventType]);
        const isExists = events.find(event => {
            if (event[eventType]) {
                const key = utils_1.getEventKey(eventType, event[eventType]);
