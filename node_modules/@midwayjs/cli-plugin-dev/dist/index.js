...
class DevPlugin extends command_core_1.BasePlugin {
        ...
        if (!this.options.entryFile || this.options.detectPort) {
            ...
            if (port !== this.port) {
                if (!this.options.silent) {
                    this.log(`Server port ${this.port} is in use, now using port ${port}`);
                }
                this.port = port;
            }
        }
        ...
        return new Promise(async (resolve) => {
            ...
            if (options.ts) {
                if (options.fast === 'esbuild') {
                    execArgv = ['-r', (0, path_1.join)(__dirname, '../js/esbuild-register.js')];
                }
                else {
    ...
    displayStartTips(options) {
        ...
        this.log('Start Server at ', chalk.hex('#9999ff').underline(`${protocol}://127.0.0.1:${options.port}`));
        const lanIp = this.getIp();
        if (lanIp) {
            this.log('Start on LAN', chalk.hex('#9999ff').underline(`${protocol}://${lanIp}:${options.port}`));
        }
        this.core.cli.log('');
        this.core.cli.log('');
    }
