...
class TestPlugin extends command_core_1.BasePlugin {
    ...
    async run() {
        ...
        const isTs = this.options.ts || fs_1.existsSync(path_1.join(cwd, 'tsconfig.json'));
        let testFiles = [];
        if (this.options.f) {
        ...
        return command_core_1.forkNode(binFile, args, opt);
    }
    async formatJestTestArgs(isTs, testFiles) {
        ...
        args.push(`--config=${path_1.join(__dirname, `../config/${isTs ? 'jest.ts.js' : 'jest.js'}`)}`);
        return args;
    }
    async formatMochaTestArgs(isTs, testFiles) {
        ...
        const setupFile = path_1.join(process.cwd(), `test/.setup.${isTs ? 'ts' : 'js'}`);
        if (fs_1.existsSync(setupFile)) {
