...
const getDevProxyMiddlewaresArr = async (options) => {
    const { fePort, proxy, isDev, https, proxyKey } = (0, loadConfig_1.loadConfig)();
    ...
    function registerProxy(proxy) {
        for (const path in proxy) {
            ...
            const middleware = express ? (0, http_proxy_middleware_1.createProxyMiddleware)(path, options) : koaConnect((0, http_proxy_middleware_1.createProxyMiddleware)(path, options));
